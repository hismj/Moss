...
def print_version():
    ...
    logger.info('作者：宋明键，孟琦，魏睿泽')
    if platform.system() == 'Windows':
        logger.info('Moss %s', ver)
        print('REST API Key: ' + api_key())
        print('Default Credentials: moss/moss')
    else:
        logger.info('%sMoss %s%s', Color.GREY, ver, Color.END)
        print(f'REST API Key: {Color.BOLD}{api_key()}{Color.END}')
        print(f'Default Credentials: {Color.BOLD}moss/moss{Color.END}')
    os = platform.system()
...
def check_basic_env():
    ...
    logger.info('Moss Basic Environment Check')
    try:
...
def get_android_dm_exception_msg():
    return (
        'Is your Android VM/emulator running? Moss cannot'
